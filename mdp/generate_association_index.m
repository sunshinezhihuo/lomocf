...
ctrack = apply_motion_prediction(frame_id, tracker);  % ctrack change

% compute distances and aspect ratios
distances = zeros(num_det, 1);
ratios = zeros(num_det, 1);
ratios_w = zeros(num_det, 1);  % add
for i = 1:num_det
    ...
    ratio_w = tracker.dres.w(end) / dres_det.w(i);  % add
    ratios_w(i) = min(ratio_w, 1/ratio_w);    % add
end
% add if end
