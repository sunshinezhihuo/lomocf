...
function [tracker,kcf_value] = LK_associate(frame_id, dres_image, dres_det, tracker,opt)
...
%kcf associate with det
det_bb = [dres_det.x;dres_det.y;dres_det.x+dres_det.w;dres_det.y+dres_det.h];

switch opt.method
    case 'KCF'
        kcf_value = kcf_association( dres_image.Igray{frame_id},det_bb,tracker );                  %----------------cf
    case 'BACF'
        kcf_value = bacf_association( dres_image.I{frame_id},det_bb,tracker,opt );                 %----------------cf
    case 'CFNet'
        peak_value = cfnet_association( dres_image.I{frame_id},det_bb,tracker,opt );                %----------------cf
        kcf_value = gather(peak_value);   % from gpuArray to nomal
    case 'lomo'
        descriptors = lomo_association( dres_image.I{frame_id},det_bb,tracker,opt );
        kcf_value = mean(descriptors);
    case 'SCT'
        kcf_value = SCT_association( dres_image.I{frame_id},det_bb,tracker,opt );     
    case 'ADNet'
        kcf_value = ADNet_association( dres_image.Igray{frame_id},det_bb,tracker,frame_id,opt ); 
    case 'lomocf'
        cf_value(1) = kcf_association( dres_image.Igray{frame_id},det_bb,tracker );
        
        % lomo
        descriptors = lomo_association( dres_image.I{frame_id},det_bb,tracker,opt );
%         desc = descriptors';
        cf_value(2) = mean(descriptors);
        
% % debug       
% figure(2)
% imshow(dres_image.I{frame_id});
% rectangle('Position', [det_bb(1) det_bb(2) (det_bb(3)-det_bb(1)) (det_bb(4)-det_bb(2))], 'EdgeColor', 'red', 'LineWidth', 2, 'LineStyle', '-');
% t1 = text(50, 50, num2str(cf_value(2)),'color','yellow');
% t1(1).FontSize = 16;
% t2 = text(15, 15, num2str(frame_id),'color','g');
% t2(1).FontSize = 14;
% hold off;       
