...
function tracker = LK_initialize(tracker, frame_id, target_id, dres, ind, dres_image, opt)  % add opt
...
switch opt.method
    case 'KCF'    
        %kcf initialize gray image
        I = dres_image.Igray{frame_id};
        tracker = kcf_initialize( I, [x1,y1,x2,y2],tracker,opt,frame_id);    %-----------------cf add opt
    case 'BACF'
        % bacf initialize color image
        I = dres_image.I{frame_id};                             % bacf color image
        tracker = kcf_initialize( I, [x1,y1,x2,y2],tracker,opt,frame_id);    % ----------------cf add opt
    case 'CFNet'
        I = dres_image.I{frame_id};
        I = single(I);        
        if(size(I, 3)==1)
            if ~isempty(tracker.net_gray)
                    tracker.net = tracker.net_gray;
                    tracker.grayscale = true;
            end
        end
    
        tracker = kcf_initialize( I, [x1,y1,x2,y2],tracker,opt,frame_id);   %-------------------cfnet
        
    case 'lomo'
        I = dres_image.I{frame_id};                             %  color image
        tracker = kcf_initialize( I, [x1,y1,x2,y2],tracker,opt,frame_id);    % ---------------- []option    
