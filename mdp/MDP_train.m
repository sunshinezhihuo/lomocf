...
if nargin < 2 || isempty(tracker) == 1
    ...
    tracker = MDP_initialize(I, dres_det, labels, opt);      %-------------------------CF
else
    % continuous training
...
while 1
    ...
    while fr <= seq_num
        ...
        elseif tracker.state == 1
            ...
            tracker = LK_initialize(tracker, fr, id, dres, ind, dres_image, opt); %---------------------------CF, I add opt
            tracker.state = 2;
        ...
        elseif tracker.state == 2      
            tracker.streak_occluded = 0;
            tracker = MDP_value(tracker, fr, dres_image, dres, [], opt);          %----------------------------cf add opt
        ...
        %debug
%         if(numel(tracker.dres.fr)>1)
%             KCFvsLK(tracker,fr,dres_gt);
%         end
